...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_delete:
                // Pop up hiển thị khi bấm xóa thiết bị
    ...
    private void deleteMobile() {
        // Chỉ xóa khi thiết bị đã tồn tại.
        if (mCurrentMobileUri != null) {
            // Gọi ContentResolver để xóa thiết bị tại content URI đã cho.
            // Truyền null vào selection và selection args vì mCurrentMobileUri
            // content URI đã xác định được thiết bị ta cần.
            int rowsDeleted = getContentResolver().delete(mCurrentMobileUri, null, null);
            // Hiển thị một toast phụ thuộc vào việc xóa thiết bị thành công hay không.
            if (rowsDeleted == 0) {
                // Nếu không có hàng nào bị xóa thì đã xảy ra lỗi.
                Toast.makeText(this, getString(R.string.editor_delete_mobile_failed),
                        Toast.LENGTH_SHORT).show();
            } else {
                // Nếu không, thì đã xóa thành công và ta sẽ hiển thị toast.
